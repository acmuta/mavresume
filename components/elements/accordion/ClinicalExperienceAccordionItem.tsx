...
interface ClinicalExperienceAccordionItemProps {
  index: number;
  entries: Experience[];
}
...
> = ({ index, entries }) => {
  ...
  return (
    <AccordionItem value={`ClinicalExperience-${index}`}>
        ...
        <div className="flex flex-1 items-center justify-between gap-4 pr-4">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-[#89a5ff]">
              Clinical Entry {index + 1}
            </p>
            <p className="mt-2 text-left text-lg font-semibold text-white">
              {entries[index]?.position || "New clinical experience entry"}
            </p>
          </div>
          {entries.length > 1 && (
            <button
              ...
              aria-label="Delete clinical experience entry"
            >
              <X className="size-4" />
            </button>
          )}
      ...
      <AccordionContent>
        <div className="grid gap-4">
          <div className="grid gap-5 md:grid-cols-2">
            <CustomTextField
              id={`clinical-role-${index}`}
              label="Role"
              placeholder="Clinical Role"
              value={entries[index].position}
              onChange={(e) => {
                updateClinicalExperience(index, { position: e.target.value });
              }}
            />
            <CustomTextField
              id={`clinical-site-${index}`}
              label="Clinical Site"
              placeholder="Hospital or Clinical Site"
              value={entries[index].company}
              onChange={(e) => {
                updateClinicalExperience(index, { company: e.target.value });
              }}
            />
